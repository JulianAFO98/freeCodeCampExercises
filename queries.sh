...
echo  "$($PSQL "SELECT name FROM teams LEFT JOIN games ON teams.team_id=games.opponent_id WHERE year=2014 AND round='Eighth-Final'")" 
echo -e "\nList of unique winning team names in the whole data set:"
echo  "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON teams.team_id=games.winner_id WHERE winner_goals IS NOT NULL ORDER BY name")"
